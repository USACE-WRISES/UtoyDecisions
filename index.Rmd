...
- Ecological data are being quality controlled and have known errors at this point (i.e., equal riparian areas for left and right bank). Additionally, we need to revisit whether year-0 should be equal to the FWOP for all alternatives.  
...
eco <- read.csv("Utoy_Phase 2_EcoInputs_2024-07-24_EcoIn.csv", header=TRUE, dec=".", na.strings="NA", stringsAsFactors = FALSE)

#Import cost data
cost <- read.csv("Utoy_Phase 2_EcoInputs_2024-07-26_CostIn.csv", header=TRUE, dec=".", na.strings="NA", stringsAsFactors = FALSE)
```
...
#Add legend
legend("bottomleft", legend = c("FWOP", "Alternative1"), lwd = 2, col = c("black", "blue"), bg = "white")
...
colnames(cost_present) <- c("ReachID", "SiteAction", "ProjectFirst", "Monitoring", "AdMan", "IDC", "OM")
  ...
  for (i in 1:nactions.total){
    cost_present$OM[i] <- om_distribute(discount, 50, 10, cost$OM_Annual[i])
  }
