...
colnames(Table04) <- c("Site", "Alternative", "Ecological Outputs (AAHU)", "Ecological Lift (AAHU)", "Project First Cost", "Annualized Cost", "Unit Cost")
...
This is just placeholder text. Describe the site and alternatives. **EXPECTED Alt 1 and 3 to be competitive. NEED TO REVISIT BENEFITS AND DOUBLE-CHECK AREAS.**  
...
- Alternative3: Chain of wetlands. Likely to have large hydrologic and sediment capture benefits, which would protect downstream investments. Line of sight in the golf course would not be impeded. 

Alternatives 1 and 2 may be more compelling in light of connectivity to 17D2E. 

**RECOMMENDATION: (Tentatively) Alternative3**
...
- Alternative1: Concrete channel removal with left bank wetland. Large lift. Direct effect on cause of ecological degradation. Low unit cost relative to watershed-scale actions.   
...
- FWOP: Future WithOut Project.  
- Alternative1: Channel realignment.  
- Alternative2: Channel shaping with bankfull bench.  
- Alternative3: Minor instream structures.
...
- FWOP: Future WithOut Project.  
- Alternative1: Bridge replacement with extensive action.  
- Alternative2: Brdige replacement with minor action.  
- Alternative3: Bank stabilization.
...
- FWOP: Future WithOut Project.  
- Alternative1: Large wood features for 50% of reach.  
- Alternative2: Large wood features for 25% of reach.  
- Alternative3: Beaver reintroduction.
...
- FWOP: Future WithOut Project.  
- Alternative2: Floodplain bench for floodplain connectivity.  
- Alternative3: Regarde segment around culvert.
...
colnames(Table06) <- c("Plan", AAHU$ReachID[find.site.data2], "Ecological Lift", 
                       "Average Annual Cost", "Project First Cost", "Unit Cost", "Incremental Unit Cost")
...
TableB04 <- cbind(sys.CEICA[,1:8], 
                 ...
                 sys.CEICA[,12:13])
colnames(TableB04) <- colnames(sys.CEICA)
